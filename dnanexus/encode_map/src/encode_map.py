...
TRIMMOMATIC_PATH = '/trimmomatic/trimmomatic-0.36.jar'
...
def postprocess(indexed_reads, unmapped_reads, reference_tar, debug):
    ...
    if paired_end:
        ...
        steps = [
            "%s sampe -P %s %s %s %s %s"
            % (BWA_PATH, reference_filename, reads1_filename, reads2_filename,
    ...
    with open(raw_bam_mapstats_filename, 'w') as fh:
        subprocess.check_call(
            shlex.split("%s flagstat %s" % (SAMTOOLS_PATH, raw_bam_filename)),
            stdout=fh)
...
def process(reads_file, reference_tar, bwa_aln_params, debug):
    ...
    with open(sai_filename, 'w') as sai_file:
        ...
        bwa_command = "%s aln %s -t %d %s %s" \
            % (BWA_PATH, bwa_aln_params, cpu_count(),
...
def main(reads1, crop_length, hard_crop, reference_tar,
         bwa_aln_params, debug, reads2=None):
